...
from Knightsx import app  # Updated project name
from Knightsx.misc import SUDOERS
from Knightsx.utils import get_readable_time
...
@language
async def ungbanuser(client, message: Message, _):
    if not message.reply_to_message:
        if len(message.command) != 2:
            return await message.reply_text(_["general_1"])
        user = message.text.split(None, 1)[1]
...
async def gbanned_list(client, message: Message, _):
    ...
    for user_id in users:
        ...
        try:
            ...
            name = user.mention if user.mention else user.first_name
            msg += f"**{count}.** {name}\n"
        except Exception:
